...
def amount_to_text_fr(Montant):
        ...
        cents_name = (cents_number > 1) and ' MAD' or 'MAD'
        final_result = start_word +end_word +cents_name +' et '+ str(cents_number)+' '+'Centimes'
        return final_result
...
@app.get("/")
async def root():
    return ("Welcome to amount to currency API!")

@app.get("/Amount_to_text")
async def amount_to_currency(amount: int = 0):
    currency =amount_to_text_fr(amount)
